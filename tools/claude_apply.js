...
// --- helpers ---
function lf(s){ return s.replace(/\r/g,''); }
...
async function callAnthropic(model, system, user){
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model, max_tokens: 4000, temperature: 0, system,
      messages: [{ role:'user', content: user }],
    }),
  });
  const data = await res.json();
  if (data.error) { const err = new Error(JSON.stringify(data.error)); err.name = data.type||'api_error'; throw err; }
...
async function anthropicWithFallback(system, user){
  ...
  for (const m of models){
    ...
    catch(e){ last = e; if (/not.*found|unsupported|permission|access/i.test(String(e))) continue; throw e; }
  }
